...
// Powered by static database
// Everything inside /alternative folder
...
// ============================================
// Static Food Alternative Database (Fallback)
// Each entry has:
//   alternative  – comparison card data (packagedIssues, benefits)
//   alternatives – array of 2 homemade recipe cards
// ============================================
const foodDatabase = [
  {
...
app.post('/api/alternative', async (req, res) => {
  try {
    ...
    if (!foodName?.trim()) return res.status(400).json({ error: 'Please provide a food name.' });

    // Find in static database
    const match = findMatch(foodName);
    if (match) {
      console.log('✅ Database match found for:', foodName);
      return res.json({
    ...
    return res.status(404).json({
      error: `No alternative found for "${foodName}". Try a different name or product.`
    });
  } catch (err) {
...
app.get('/api/health', (req, res) => {
  res.json({ status: 'ok', db_size: foodDatabase.length });
});
...
app.listen(PORT, () => {
  ...
  console.log(`  Database:  ${foodDatabase.length} foods loaded`);
  console.log(`  Open:      http://localhost:${PORT}/alternative.html\n`);
});